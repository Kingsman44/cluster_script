...
collect_metrics() {
    ...
    for cluster in $CLUSTERS; do
        ...
        ram_value="$(echo "$var" | grep "$cluster " | awk '${print $2}' | tr -d '%')"
        rtt_value=$(ping -c 1 $cluster | tail -1 | awk '{print $4}' | cut -d'=' -f2)
...
#         ram_value="$(echo "$var" | grep "$pod " | awk '${print $2}' | tr -d '%')"
...
#     done
    ...
    for value in $saved_cluster_value; do
        cluster="$(echo $value | cut -d, -f1)"
        var="$(kubectl top node $cluster)"
        cpu_value="$(echo "$var" | grep "$cluster " | awk '${print $3}' | tr -d '%')"
        ram_value="$(echo "$var" | grep "$cluster " | awk '${print $2}' | tr -d '%')"
        rtt_value="$(ping -c 1 $cluster | tail -1 | awk '{print $4}' | cut -d'=' -f2)"
        final_value="$value,$cpu_value,$ram_value,$rtt_value"
        echo CLUSTER:$final_value
